...
object GeminiService {
    ...
    fun isKeyConfigured(): Boolean {
        return apiKey.isNotEmpty() && apiKey != "MY_GEMINI_API_KEY" && apiKey != "placeholder_key"
    }

    suspend fun generateResponse(prompt: String, systemPrompt: String? = null): String = withContext(Dispatchers.IO) {
        if (!isKeyConfigured()) {
            return@withContext "[SIMULATED RESPONDER NODE]: API keys offline. Emulation Matrix active for node connection."
        }
        ...
        try {
            ...
            response.candidates?.firstOrNull()?.content?.parts?.firstOrNull()?.text
                ?: "[SYSTEM ERROR: Decryption Failed]"
        } catch (e: Exception) {
            "[SANDBOX ERROR: ${e.localizedMessage ?: "Isolated Exception"}]"
        }
    }
}
